...
const SectionHeading = ({
  ...
  return (
    <div className="relative">
      <h2
        className={`font-apercu-black text-[15rem] uppercase font-semibold select-none text-center ${className ? className : "max-sm:text-[4.5rem] sm:max-md:text-[7.5rem] md:max-lg:text-[8rem] lg:max-xl:text-[10rem]"}`}
        style={{
          maskImage:
      ...
      >
        {headingText.split("").map((letter, index) => (
          <span
            key={index}
            className={`${index === lastIndex ? "" : "mr-14 max-sm:mr-2 sm:max-md:mr-5 md:max-lg:mr-5 lg:max-xl:mr-8"}`}
          >
            {letter}
          </span>
        ))}
