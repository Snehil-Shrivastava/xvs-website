...
const AboutProcess = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-10 max-md:w-9/10 sm:max-md:max-w-150 max-md:mx-auto max-md:pb-40">
      <SectionHeading
        headingText="Process"
        desc="We keep it simple and listen..."
      />
      <div className="max-w-450 max-md:max-w-full mx-auto flex max-md:flex-col gap-10">
        {aboutProcess.map((process, index) => (
          <GlowCard
            key={index}
            className={`backdrop-blur-sm w-180 max-md:w-full ${index === 0 ? "container-clip-tl" : "container-clip-br"}`}
            cardStyle={`bg-[radial-gradient(circle_at_bottom_right,_rgb(15,15,15,0.5),_rgba(35,35,35,0.5))] backdrop-blur-md px-15 max-sm:px-8 py-15 max-md:py-8 ${index === 0 ? "content-clip-tl" : "content-clip-br"}`}
            contentStyle="flex flex-col max-md:gap-5"
          >
            <div className="flex items-center">
              <div
                className={`relative h-32 w-33 max-sm:h-10 max-sm:w-11 sm:max-md:h-20 sm:max-md:w-26`}
              >
                <Image
